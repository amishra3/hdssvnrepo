...
<div class="pr-explore news-explore" id="${properties.id}"
	style="background-image: url('${properties.newsbackgroundimage}')">
	<div class="pr-explore-container">
		<div class="row">

			<c:forEach items="${newsExplorerVerticalModel.nevcList}"
					...
					<div class="col-sm-4">
						...
						<c:choose>
							<c:when test="${fn:substringAfter(nevcMultiObject.value, '$') == '1'}">
								<h2>
									<a href="${hdscorp:shortURL(fn:substringBefore(nevcMultiObject.value, "$"))}" target="_blank">${nevcMultiObject.key} <span
										class="glyphicon glyphicon-new-window" aria-hidden="true"></span></a>
								</h2>
							</c:when>
							<c:otherwise>
								<h2>
									<a href="${hdscorp:shortURL(fn:substringBefore(nevcMultiObject.value, "$"))}" class="animateLink">${nevcMultiObject.key} <span
						...
						<c:choose>
							<c:when test="${properties.numberofnewsrows=='1'}">
								<cq:include path="newsfirstpar${multfieldStatus.count}"
									resourceType="/apps/hdscorp/components/content/newsverticalexplorer" />
							</c:when>
							<c:otherwise>
								<cq:include path="newsfirstpar${multfieldStatus.count}"
									resourceType="/apps/hdscorp/components/content/newsverticalexplorer" />
								<cq:include path="newssecondpar${multfieldStatus.count}"
									resourceType="/apps/hdscorp/components/content/newsverticalexplorer" />
							</c:otherwise>
						</c:choose>
					</div>
