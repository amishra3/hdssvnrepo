...
<div class="description"> ${blogModel.bcontent}</div>
	<c:choose>
     <c:when test="${not empty blogModel.breadmore}">
         <div class="read-more">
               <a href="${blogModel.breadmorelink}" class="animateLink" target="${blogModel.bopeninnew?'_blank':'_self'}">${blogModel.breadmore} <span class="glyphicon glyphicon-menu-right animateIcon" aria-hidden="true"></span></a>
          </div>
       </c:when>
        <c:otherwise>
        </c:otherwise>
