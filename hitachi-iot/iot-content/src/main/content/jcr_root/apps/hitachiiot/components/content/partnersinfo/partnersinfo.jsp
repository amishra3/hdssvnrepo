            ...
            <c:forEach var="partner" items="${partners}">
               <div class="partner col-xs-6 col-sm-6 col-md-2 col-lg-2">
				  ...
				  <c:set var="linkUrl" value="${partner.partnericonurl}" />
					...
					<a href="${fn:contains(linkUrl, 'http')?'':domain}${linkUrl}" target="${partner.icontargettype?'_blank':'_self'}" title="${partner.partnericonalttext}">
				  
                     <img src="${partner.partnericonimgpath}" alt="${partner.partnericonalttext}" title="${partner.partnericonalttext}" class="img-responsive">
					 </a>
                  </div>
               </div>
            </c:forEach>
