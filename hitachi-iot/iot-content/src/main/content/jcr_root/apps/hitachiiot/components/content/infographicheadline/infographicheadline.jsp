			...
			<h2>${properties.headlinecontent}</h2>
			 ${properties.descriptioncontent}
           	<c:if test="${not empty buttonurl}">
				<a href="${buttonurl}" class="btn-square-red" target="${properties.buttontargettype==true?'_blank':'_self'}">${properties.buttonlabel}${properties.showthirdpartyicon?' <span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>':''}</a>
            </c:if>
    	</div>
	</div>
</div>
