...
(function($) {
	...
	$(document).ready(function(){
			...
			$("body").append('<svg version="1.1" xmlns="http://www.w3.org/2000/svg">'+
								'<mask id="slotMachineFadeMask" maskUnits="objectBoundingBox" maskContentUnits="objectBoundingBox">'+
									'<linearGradient id="slotMachineFadeGradient" gradientUnits="objectBoundingBox" x="0" y="0">'+
										'<stop stop-color="white" stop-opacity="0" offset="0.25"></stop>'+
		...
		$("body").append("<style>"+
								...
								".slotMachineBlurMedium{-webkit-filter: blur(2px);-moz-filter: blur(2px);-o-filter: blur(2px);-ms-filter: blur(2px);filter: blur(2px);filter: url(#slotMachineBlurFilterMedium);filter:progid:DXImageTransform.Microsoft.Blur(PixelRadius='2')}"+
	...
	$.fn.slotMachine = function(settings){
		...
		*/
		...
		/**
		  ...
		  * @param int||boolean nowOrRepeations - Number of repeations to stop (true to stop NOW)
		  * @param int stop_at_index forcing the slotmachine to stop at this index.
		*/
		$slot.stop = function( nowOrRepeations, stop_at_index ){
			...
			var stopAtIndexFunction = null;
			if(typeof stop_at_index == 'number'){
				stopAtIndexFunction = function(){
					var nextObj = {
						index	: stop_at_index,
						el		: $titles.get(stop_at_index)
					};	
					return nextObj;

				}
			}
			_stop(nowOrRepeations, stopAtIndexFunction);
