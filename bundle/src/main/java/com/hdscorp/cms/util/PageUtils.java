...
public final class PageUtils {
	 ...
	 */
	private static String getStringFromArrayProperty(final Object[] objectArray, final String propertyName,
			final int index) {
	...
	public static Boolean createNodesForJSON(final String nodeName, final Node currentNode,
		...
		try {
			...
			if (currentNode == null) {
				...
				final Node node = JcrResourceUtil.createPath(nodePath, "nt:unstructured", "nt:unstructured", session,
						false);
				if (node != null) {
					node.setProperty("sling:resourceType", "hdscorp/components/content/" + nodeName);
	...
	public static String getComponentPath(final Page currentPage, ResourceResolver resourceResolver, String parent,
		...
		try {
			...
			String nodePath = currentPage.getPath() + "/jcr:content/" + parent + "/" + componentName;
